...
for _, scrip in ipairs(tree.tree) do
    if scrip.type == "blob" and strip.path ~= "Loader.lua" and string.split(string.split(scrip.path:lower(), ".lua")[1]:lower(),game:GetService("MarketplaceService"):GetProductInfo(game.PlaceId).Name:lower())[1] ~= game:GetService("MarketplaceService"):GetProductInfo(game.PlaceId).Name:lower() then
